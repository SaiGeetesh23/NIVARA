...
# rag_agent_obj = RAGAgent(rag_llm, embedding, ZILLIZ_CLOUD_URI, ZILLIZ_CLOUD_USERNAME, ZILLIZ_CLOUD_PASSWORD).create()
# market_agent_obj = MarketAgent(market_llm).create()
# planner_agent_obj = PlannerAgent(planner_llm).create()
...
# app.add_middleware(
#     CORSMiddleware,
#     allow_origins=["*"],  # NOTE: Restrict to specific origins in production
...
async def lifespan(app: FastAPI):
    ...
    app.state.nivara_graph = create_supervisor(
        model=supervisor_llm,
        agents=[rag_agent_obj, market_agent_obj, planner_agent_obj],
        prompt=supervisor_agent_prompt,
    ).compile(name="nivara_supervisor", checkpointer=memory)
...
async def generate_chat_response(message: str, thread_id: str, app: FastAPI):
    config = {"configurable": {"thread_id": thread_id}}

    async for event in app.state.nivara_graph.astream_events(
...
async def chat_stream(
    ...
    thread_id = current_user.thread_id
    return StreamingResponse(
        generate_chat_response(chat_request.message, thread_id, request.app),
        media_type="text/event-stream",
    )
