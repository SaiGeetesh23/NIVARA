...
const ChatPage = () => {
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
                ...
                for (const line of lines) {
                    if (line.startsWith('data: ')) {
                        ...
                        try {
                            ...
                            const aiId = aiResponsePlaceholder.id;
                            if (data.type === 'tool_start') {
                                setMessages(prev => prev.map(msg => msg.id === aiId ? { ...msg, toolStatus: data.content, content: streamedContent } : msg));
                            } else if (data.type === 'tool_end') {
                                setMessages(prev => prev.map(msg => msg.id === aiId ? { ...msg, toolStatus: undefined } : msg));
                            } else if (data.type === 'content') {
                                streamedContent += data.content;
                                setMessages(prev => prev.map(msg => msg.id === aiId ? { ...msg, content: streamedContent, isLoading: true } : msg));
                            } else if (data.type === 'end') {
                                setMessages(prev => prev.map(msg => msg.id === aiId ? { ...msg, isLoading: false, toolStatus: undefined } : msg));
            ...
            setIsStreaming(false);
            setMessages(prev => prev.map(msg => msg.id === aiResponsePlaceholder.id ? { ...msg, isLoading: false, toolStatus: undefined } : msg));
        }
    };
